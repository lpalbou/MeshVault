...
| `GET` | `/api/asset/file` | Serve a 3D file from the filesystem (auto-converts old FBX) |
| `GET` | `/api/asset/archive` | Extract and serve a 3D file from an archive |
| `GET` | `/api/asset/prepare_archive` | Extract an archived asset and return resolved temp paths |
| `GET` | `/api/asset/related` | Serve a related file (material, texture) |
| `POST` | `/api/export` | Export an asset with a new name to a target directory |
