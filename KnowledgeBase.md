...
## Agora seat + MCP endpoint boot (2026-07-19)

- **Seat**: `meshvault` on the local agora hub (127.0.0.1:8765), member of `commons` (joined 2026-07-19 on operator instruction — the seat's mandate is to assist other agents needing 3D capabilities). Positioning per operator: meshvault is an INDEPENDENT application, deliberately NOT an AbstractFramework package (open to discussion later); it complements `abstract3d` (they GENERATE, we VIEW/EDIT/VERIFY/REPAIR).
- **Endpoint state on this machine**: `meshvault-mcp` is stdio spawn-on-demand — "up" means SPAWNABLE, not a daemon. Installed editable into `.venv/` (`pip install -e ".[mcp]"`, python 3.12.13 via pyenv; playwright chromium already cached in `~/Library/Caches/ms-playwright`). Registered in `.cursor/mcp.json` as `meshvault` with the absolute `.venv/bin/meshvault-mcp` path (PATH does not carry it).
- **Endpoint verification method**: `scripts/mcp_smoke.py` — spawns the server through the real MCP client, lists tools (13), runs add_primitive → sculpt → fill_paint → describe_scene, dumps `list_viewer_commands` (117 commands) for cross-checks. Exit 0 = functional. Re-run after dependency or server changes.
- **Skill freshness check (do this when the tool surface changes)**: every action named in `.cursor/skills/meshvault-live-editing/SKILL.md` must exist in the live `list_viewer_commands` dump; spot-check enums (paint_pattern types, deform_region kinds, sculpt tools). Verified in full 2026-07-19: 35/35 named actions present, enums match.
- **Skill registration question**: opened with the `skill` seat (DM, 3 asks: non-framework skills policy; evidence/version-drift handling — repo-as-source + pinned tree hash vs shelf-owned bytes; declaring a required MCP server for loud activation failure). ANSWERED same evening, adopted on the record (DM seq 3/4).
- **Fable5 admission verdict on the skill (2026-07-19, DM seq 7)**: ADMIT-AFTER-FIXES. The P1 was OURS to learn from: the terminal Verification battery said "fix_mesh {degenerate}, expect 0 drops" while the Fusion section forbids exactly that on fused meshes — an agent following the checklist literally would re-enact the documented seam-punching incident. Rule: a skill's TERMINAL checklist must carry every exception its own body establishes (checklists get followed without re-reading the sections that qualify them). Riders fixed same-pass: golden-rules renumbered (the rhetorical "0." item was invisible in CommonMark rendering), "7b." lazy-merged into item 7 (invalid list marker), repo-file reference dangled once vendored (single-file skill must not cite repo paths), install line reworded to operator-lane, probe-batch arithmetic corrected (70 pts / 32 per call = 3 round trips). FOLLOW-UP (non-blocking, skill's fleet bench): activation charge is ~36.6 KB / ~9k tokens — a core+references split would relieve small-window substrates (their 11.5 KB skill overflowed 4B-class windows).
- **Degenerate-count is a LOADED-VIEW measure, not a file-bytes measure (first verification job, 2026-07-19)**: my `get_mesh_stats` degenerate count reported 0 on abstract3d's standalone modified.glb but 2 on the byte-same geometry embedded in their scene.glb — the standalone's generation-stage thin triangles only collapsed to exact zero area after their compose step baked node transforms into float32 vertex data (abstract3d root-caused + fixed same-hour: compose now keeps vertex data verbatim and places via scene NODE transforms; their raw `process=False` reads see 2 degens in BOTH). Lesson for verification passes: a 0-vs-N degenerate delta between "same" geometries can be the WRITE PIPELINE (quantization/baking), and my count reflects the viewer's loaded view — corroborate cross-file deltas with a raw-bytes read (or the producer's own raw reader) before attributing them to either side.
- **abstractskill shelf registration contract (2026-07-19, skill's answer — binding for our lane)**: the shelf is CURATED-only, not framework-only; third-party skills cap at method=manual-review / level=adopted (code-enforced). We keep OWNERSHIP (repo = source of truth); the shelf takes a catalog entry `{name, source repo, upstream_ref = 40-hex commit SHA (immutable — tags/branches refused), activation_description naming the meshvault-mcp dependency, notes with evidence receipt}` and vendors bytes verbatim (tree-hash pinned; fable5 adversarial review before vendoring). Knowledge-pack constraint: NO `scripts/` dir in the skill (flips has_scripts → forces requires_review) — keep the skill a single SKILL.md. DRIFT DISCIPLINE: when a release changes the taught surface, FLAG on the hub the turn it ships; re-vendoring is deliberate (new SHA, delta re-review), never automatic — `scripts/mcp_smoke.py` is the mechanical freshness check backing drift flags. Dependency declaration conventions (no first-class field yet): description names the server + `metadata: {requires_mcp: [meshvault-mcp]}` (skill seat is filing the host-side loud-refusal check; our skill is its first consumer) + a prerequisites paragraph at the top of the body. Registration pending exactly ONE gate: an operator-approved commit so the convention edits get an immutable SHA to pin.
